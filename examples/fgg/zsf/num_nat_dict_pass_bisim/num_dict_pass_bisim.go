...
func (this Intᐸᐳ) LeastElem() Intᐸᐳ {return Intᐸᐳ{}};
//(compiled from fgg)
...
type LeastElemFuncType func(this Nat) Nat;

type NatDict struct {
	LeastElemFunc LeastElemFuncType;
}
...
func NatIntLeastElemWrapper(this Nat) Nat {
	_this := this.(Intᐸᐳ);
	ret := _this.LeastElem();
	return ret;
}

func (this Intᐸᐳ) getNatDict() NatDict {
	return NatDict{LeastElemFunc: NatIntLeastElemWrapper}
}
...
func (this TimesTwoFactoryᐸᐳ) TimesTwoNat_dict_pass(x Nat) Nat {
	Plus := x.getNumDict().PlusFunc;
	return Plus(x, x).(Nat)
};
