...
//I-DICTS
func (this DictPassTranslator) MakeDictForLists(ds []fgg.Decl, eta map[fgg.TParam]fg.FGExpr, delta fgg.Delta, tau []fgg.Type, psi fgg.BigPsi) []fg.FGExpr {
	ret := []fg.FGExpr{}
	l := len(tau)
	for i := 0; i < l; i++ {
		tau_i := tau[i]
		psi_i := psi.TFormals[i].U_I
		ret = append(ret, this.makeDict(ds, eta, delta, tau_i, psi_i))
	}
	return ret
}

func (this DictPassTranslator) makeDict(ds []fgg.Decl, eta map[fgg.TParam]fg.FGExpr, delta fgg.Delta, tau fgg.Type, smalldelta fgg.Type) fg.FGExpr {
	switch tau_ := tau.(type) {
	case fgg.TNamed: //I-NEWDICT
		meths, order := this.env.MethodsDelta(delta, smalldelta)
		...
		println("smalldelta: ", smalldelta.String())
		if !no_assertion_global && (!this.env.OptAssertionForZeroParam || this.env.OptAssertionForZeroParam && !this.env.CanOptimizedOutTypeMetadata(smalldelta)) {
			if this.env.OptAssertionForZeroParam {
				es = append(es, AuxTypeMeta(composeEtaToZetaDummy(eta), tau_))
			} else {
	...
	case fgg.TParam:
		if fgg.IsNamedIfaceType(ds, smalldelta) { //I-VARDICT
			...
			panic("small delta should not be a struct")
	...
	case fgg.ChannelType:
		panic("chan type will not subtype to interface. This wouldn't happen.")
...
func (this DictPassTranslator) TranslateMethod(ds []fgg.Decl, decl fgg.MethDecl) []fg.Decl {
	this.env.EnterMethod()
	defer this.env.ExitMethod()
	//rule out Phi != Phi'
	...
	for _, ty := range decl.Psi_meth.TFormals {
		println("TFormals", ty.U_I.String())
		if this.env.OptAssertionForZeroParam && this.env.CanOptimizedOutTypeMetadata(ty.U_I) {
			println("nil")
			spec_struct_lit_es = append(spec_struct_lit_es, fg.NewVariable("nil"))
		} else {
			spec_struct_lit_es = append(spec_struct_lit_es, AuxTypeMeta(zeta, ty.U_I))
		}
	}
	for _, ty := range decl.GetParamDecls() {
		if this.env.OptAssertionForZeroParam && this.env.CanOptimizedOutTypeMetadata(ty.U) {
			spec_struct_lit_es = append(spec_struct_lit_es, fg.NewVariable("nil"))
		} else {
	...
	if !this.env.No_assertion {
		if this.env.OptAssertionForZeroParam {
			spec_methbody.StmtBody = []fg.FGStmt{fg.OtherStmt{"panic(\"method should not be used.\")"}}
		}
