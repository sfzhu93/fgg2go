...
func NewAssert(e FGExpr, t TypeBase) Assert           { return Assert{e, t} }
...
func (x Variable) Typing(ds []Decl, gamma Gamma, allowStupid bool) TypeBase {
	res, ok := gamma[x.name]
	if !ok {
...
func (s StructLit) Typing(ds []Decl, gamma Gamma, allowStupid bool) TypeBase {
...
func (s Select) Typing(ds []Decl, gamma Gamma, allowStupid bool) TypeBase {
...
func (c Call) GetReceiver() FGExpr { return c.ExprRecv }
...
func (c Call) Eval(ds []Decl) (FGExpr, string) {
	if !c.ExprRecv.IsValue() {
		e, rule := c.ExprRecv.Eval(ds)
...
func (c Call) Typing(ds []Decl, gamma Gamma, allowStupid bool) TypeBase {
	t0 := c.ExprRecv.Typing(ds, gamma, allowStupid)
	var g Sig
...
func (c Call) CanEval(ds []Decl) bool {
	if c.ExprRecv.CanEval(ds) {
		return true
	} else if !c.ExprRecv.IsValue() {
		return false
	}
...
func (c Call) String() string {
	var b strings.Builder
	b.WriteString(c.ExprRecv.String())
	b.WriteString(".")
...
func (c Call) ToGoString(ds []Decl) string {
	var b strings.Builder
	b.WriteString(c.ExprRecv.ToGoString(ds))
	b.WriteString(".")
	b.WriteString(c.meth)
	b.WriteString("(")
...
func (a Assert) GetType() TypeBase { return a.t_cast }
...
func (a Assert) Typing(ds []Decl, gamma Gamma, allowStupid bool) TypeBase {
...
func (s StringLit) Typing(ds []Decl, gamma Gamma, allowStupid bool) TypeBase {
	return STRING_TYPE
}
...
func (s Sprintf) Typing(ds []Decl, gamma Gamma, allowStupid bool) TypeBase {
	for i := 0; i < len(s.args); i++ {
